...
class Dao_MatchGoods {
	...
	public function doMatchGoods($arrInput){

		$name = $arrInput["name"];

		$result_select = $this->db_client->select('model_goods_info','goods_id,name,ser_num,super_id,spec,unit,instock_price,instock_count,instock_avg,category,reference_price,reference_cost',
			' name like "%'.$name.'%"',NULL,' limit 0,100');
			//' name like "%'.$name.'%"',NULL,'order by length(name)-length(replace(name,"'.$name.'",""))');
