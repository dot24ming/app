...
class Tool_WeiXin {
	...
	const EMPLOYEE_TEAM = 101;
	const CUSTOMER_TEAM = 102;
	const TEST_TEAM = 100;
	...
	public static function sendTempMsg($msg)
	{
		$url = 'https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=%s';
		$accessToken = self::getToken();
		Bd_Log::warning("accessToken:".$accessToken);
	...
	public static function sendTempMsg_X($msg)
	{
		$url = 'https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=%s';
		$accessToken = self::getToken_X();
		Bd_Log::warning("accessToken:".$accessToken);
		$url = sprintf($url, $accessToken);
		$data = json_encode($msg);
		$cmd = "curl $url -d '$data'";
