...
class Action_Quote extends Ap_Action_Abstract {
	...
	public function execute() {
		...
		if(!isset($arrInput["form_id"]) or $arrInput["form_id"]==""){
			...
			$select_result = $db_client->select(Tool_Util::getStoreTable('model_goods_quote'),'quote_status,time','quote_id='.$arrInput["form_id"],NULL,NULL);
			if ($select_result!=false and count($select_result)>0 )
			{
				$form_status = $select_result[0]['quote_status'];
