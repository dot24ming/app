...
class Action_MemberCharge extends Ap_Action_Abstract {
	...
	public function execute() {
		...
		*/
		...
		$result_select = $db_client->select(Tool_Util::getStoreTable('model_user_info'),'member_card_balance','user_id='.$arrInput['uid'],NULL,NULL);
		if($result_select!==false and count($result_select)>0)
		{
			...
			$result_insert = $db_client->insert(Tool_Util::getStoreTable('model_member_recharge'),$param);
			if ($result_insert!==false and count($result_insert)>0)
			{
				$update_param = array(
						'member_card_balance' => $new_balance
					);
				$result_update = $db_client->update(Tool_Util::getStoreTable('model_user_info'),$update_param,'user_id='.$arrInput['uid'],NULL,NULL);
