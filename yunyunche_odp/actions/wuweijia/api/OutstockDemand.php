...
class Action_OutstockDemand extends Ap_Action_Abstract {
	...
	public function execute() {
		...
		*/
		...
		$record_count = 0;
		$select_count = $db_client->selectCount(Tool_Util::getStoreTable('model_mobile_service_form'),'quick_outstock!=1',NULL,NULL);
		$record_count += $select_count;

		$result_select = $db_client->select(Tool_Util::getStoreTable('model_mobile_service_form_info'),'form_id','quick_outstock!=1 and type=1',NULL, 'GROUP BY form_id');
		...
		$result_select = $db_client->select(Tool_Util::getStoreTable('model_mobile_service_form'),'*',' form_id in ('.$goods_form_str.') and quick_outstock!=1 ',NULL,'ORDER BY time DESC limit 0,100');
		...
		if ($result_select!==false and count($result_select)>0)
		{
			...
			$car_info_dict = array();
			$car_result_select = $db_client->select(Tool_Util::getStoreTable('model_car_info'),'*',' plate_number in ('.$plate_number_str.')',NULL,NULL);
			...
			$form_info_result_select = $db_client->select(Tool_Util::getStoreTable('model_mobile_service_form_info'),'*',' form_id in ('.$form_id_str.') and type=1',NULL,NULL);
			...
			foreach($form_info_result_select as $form_info)
			{
				...
				$goods_select_result = $db_client->select(Tool_Util::getStoreTable('model_goods_info'),'*',' goods_id='.$goods_id,NULL,NULL);
