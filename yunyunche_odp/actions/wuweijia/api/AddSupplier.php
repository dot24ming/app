...
class Action_AddSupplier extends Ap_Action_Abstract {
	...
	public function execute() {
		...
		if(!isset($arrInput["supplier_name"]) or $arrInput["supplier_name"]==""){
			...
			$result_select = $db_client->select(Tool_Util::getStoreTable('model_supplier_info'),'*','supplier_name="'.$supplier_name.'"',NULL,NULL);
			if ($result_select!=false and count($result_select)>0){
				...
				$result_insert = $db_client->insert(Tool_Util::getStoreTable('model_supplier_info'), $item_param);
