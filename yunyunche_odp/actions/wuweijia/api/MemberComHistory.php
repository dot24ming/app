...
class Action_MemberComHistory extends Ap_Action_Abstract {
	...
	public function execute() {
		...
		$record_count = 0;
		$select_count = $db_client->selectCount(Tool_Util::getStoreTable('model_mobile_service_form'),'user_id='.$arrInput['uid'].' and status=1',NULL,NULL);
		$record_count += $select_count;

		$member_com_select = $db_client->select(Tool_Util::getStoreTable('model_mobile_service_form'),'*','user_id='.$arrInput['uid'].' and status=1',NULL,'ORDER BY settlement_time DESC '.$limit_str);
		if($member_com_select!==false)
		{
			...
			if(count($form_id_list)>0)
			{
				...
				$service_detail_select = $db_client->select(Tool_Util::getStoreTable('model_mobile_service_form_info'),'*','form_id in ('.$form_id_str.')',NULL,NULL);
