...
class Action_ClientBusiness extends Ap_Action_Abstract {
	...
	public function execute() {
		...
		if ($package_select)
		{
		$select_count = $db_client->selectCount(Tool_Util::getStoreTable('package_card'),'user_id='.$arrInput['uid'],NULL,NULL);
		$record_count += $select_count;
		if($member_select)
		{
			$result_package_buy = $db_client->select(Tool_Util::getStoreTable('package_card'),'*','user_id='.$arrInput['uid'],NULL,'ORDER BY active_date DESC '.$all_limit_str);
		}
		else
		{
		...
		if($member_select)
		{
		$select_count = $db_client->selectCount(Tool_Util::getStoreTable('member_recharge'),'uid='.$arrInput['uid'],NULL,NULL);
		$record_count += $select_count;
		if($package_select)
		{
			$result_member_recharge = $db_client->select(Tool_Util::getStoreTable('member_recharge'),'*','uid='.$arrInput['uid'],NULL,'ORDER BY recharge_datetime DESC '.$all_limit_str);
		}
		else
		{
