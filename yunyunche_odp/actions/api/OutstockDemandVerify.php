...
class Action_OutstockDemandVerify extends Ap_Action_Abstract {
	...
	public function execute() {
		...
		$form_result_select = $db_client->select('model_mobile_service_form','*',' form_id='.$arrInput["form_id"],NULL,NULL);
		...
		$result_select = $db_client->select('model_mobile_service_form_info','*',' form_id='.$arrInput["form_id"].' and type=1',NULL,NULL);
		if ($result_select!==false and count($result_select)>0)
		{
			foreach($result_select as $service_goods)
			{
				...
				$goods_info_list = $db_client->select('model_goods_info','*',' goods_id = '.$s_item['goods_id'],NULL,NULL);
				...
				$total_num += $service_goods['count'];
				$total_price += $service_goods['price'] * $service_goods['count'];


				$s_item['name'] = $goods_info_list[0]['name'];
				//TODO
				//$s_item['count'] = ($service_goods['count'] - $goods_instock_count);
				$s_item['count'] = $service_goods['count'];
				...
				if(($service_goods['count'] - $goods_instock_count)>0)
				{
