...
class Action_PackageComHistory extends Ap_Action_Abstract {
	...
	public function execute() {
		...
		$member_com_select = $db_client->select(Tool_Util::getStoreTable('mobile_service_form'),'form_id,settlement_time,person,picker,plate_number','user_id='.$arrInput['uid'].' and status=1',NULL,'ORDER BY settlement_time DESC ');
		if($member_com_select!==false)
		{
			...
			if(count($form_id_list)>0)
			{
				...
				$select_count = $db_client->selectCount(Tool_Util::getStoreTable('mobile_service_form_info'),'form_id in ('.$form_id_str.')',NULL,NULL);
				$record_count += $select_count;
				$service_detail_select = $db_client->select(Tool_Util::getStoreTable('mobile_service_form_info'),'*','form_id in ('.$form_id_str.') and package_card_id>0',NULL,$limit_str);
