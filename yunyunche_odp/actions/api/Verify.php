...
class Action_Verify extends Ap_Action_Abstract {
	...
	public function execute() {
		...
		if (array_key_exists($form_type,$form_type_dict))
		{
			...
			{
				$select_form_result = $db_client->select(Tool_Const::$Verify_form_type[$form_type],'*',Tool_Const::$Verify_id_const[$form_type].'='.intval($form_id),NULL,NULL);
				if($select_form_result!==false and count($select_form_result)>0)
				{
					$form_info = $select_form_result[0];
					if(isset($form_info['maintance_id']) and $form_info['maintance_id']>0)
					{
						$service_form_id = $form_info['maintance_id'];
					}
				}
				//var_dump($service_form_id);
				//var_dump($select_form_result);



				$select_result = $db_client->select($table_name,'*',Tool_Const::$Verify_id_const[$form_type].'='.intval($form_id),NULL,NULL);
				...
				//{
					...
					$goods_select_result = $db_client->select(Tool_Util::getStoreTable('goods_info'),'*',' goods_id in ('.$goods_id_str.')',NULL,NULL);
					foreach($goods_select_result as $goods_info)
					{
						...
						if(isset($tmp_goods_dict[$goods_id]))
						{
							...
							//var_dump($goods_info);
							$result_update = $db_client->update('model_goods_info',$update_item_param,' goods_id='.$goods_id,NULL,NULL);
							//var_dump($result_update);
							if ($result_update !== false)
							{
								if($service_form_id!==NULL and (int)$service_form_id>0)
								{
									$update_service_form_param = array(
										'instock_count' => $tmp_goods_dict[$goods_id]['number'],
									);
									$result_update = $db_client->update("model_mobile_service_form_info",$update_service_form_param,' form_id='.$service_form_id,NULL,NULL);
									$update_demand_param = array(
										'purchased_count' => $tmp_goods_dict[$goods_id]['number'],
									);
									$result_update = $db_client->update("model_goods_demand",$update_demand_param,' service_id='.$service_form_id.' and goods_id='.$goods_id,NULL,NULL);
								}
							}
							else
							{
			...
			//var_dump($form_id);
			$result_select = $db_client->update(Tool_Const::$Verify_form_type[$form_type],$update_param,$update_conds_param);
			...
			if($service_form_id!==NULL and (int)$service_form_id>0)
			{
				...
				$result_update = $db_client->update("model_mobile_service_form",$update_service_form_param,' form_id='.$service_form_id,NULL,NULL);
			}
