<?php 
class Action_InstockExport extends Ap_Action_Abstract {
	public function execute() {
		...
		$list = $goodsStorageDao->getListByTime($start, $end, $total, $timeStart, $timeEnd);
		...
		for($i=0;$i<$list_size;$i++)
		{
			...
			$goodsInfos = $goodsStorageInfoDao->getInfo($storage_id);
			$list[$i]['detail'] = $goodsInfos;

			//var_dump(Tool_Const::$storage_type_e2c[$list[$i]['storage_type']]);
			$list[$i]['storage_type'] = Tool_Const::$storage_type_e2c[$list[$i]['storage_type']];
			$list[$i]['storage_status'] = Tool_Const::$form_status_e2c[$list[$i]['storage_status']];	
		}
		$data = $list;
		/*
