...
class Action_ReportBalanceSearch extends Ap_Action_Abstract {
	public function execute() {
		...
		$serviceForms = $mobileServiceFormDao->getServiceFormBySTime(1, $timeStart, $timeEnd);
		//todo:增加其他查询方式
		...
		foreach ($serviceForms as $serviceForm){
			...
			$balance['pay_man'] = $plateNum;
			$balance['time'] = $serviceForm['settlement_time'];
			$balance['balance_type'] = "收入";
			$balance['item_name'] = "汽修收入";
			$balance['price'] = $serviceForm['settlement_amount_ex'];
			$balance['settlement'] = $serviceForm['settlement_ex'];
			$balance['saleman'] = $serviceForm['person'];
			$balance['count'] = 1;
			array_push($balances, $balance);
		}
