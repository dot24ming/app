 ...
 * @author yincunxiang
 */
class Action_ServiceformAddSubmit extends Ap_Action_Abstract {
	...
	public function execute() {
        ...
        $arrInput_a = $arrRequest['post'];
		$arrInput = $arrInput_a['input'];

		Bd_Log::addNotice('input'.$arrInput);
        if(!isset($arrInput['charge'])){
        	//output error
		}
        Bd_Log::debug('request input', 0, $arrInput);
        
	    //3. call PageService
		$objServicePageServiceformAdd = new Service_Page_ServiceformAddSubmit();
		$arrPageInfo = $objServicePageServiceformAdd->execute($arrInput);
		...
		return Tool_Util::returnJson($arrOutput,$arrOutput["errno"],'');
	    ...
	    //$tpl->assign('arrOutput', $name);
	    //$tpl->display('yunyunche_odp/serviceformadd.tpl');
